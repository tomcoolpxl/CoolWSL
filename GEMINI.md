...
* Release flow: releases are tag-driven through CI/CD only; push `main`, create tag `vX.Y.Z`, and push the tag; the release workflow now generates notes from the actual tag-to-tag commit range instead of relying on GitHub auto-notes for unlabeled direct commits; after triggering a release, use one blocking `gh run watch <run_id> --repo tomcoolpxl/CoolWSL --exit-status`, then verify once with `gh release view vX.Y.Z --repo tomcoolpxl/CoolWSL` and confirm MSI, ZIP, and checksums assets are present.
* Packaging and winget: packaging is installer-first through `build/Invoke-ReleaseInstaller.ps1` and CI publishes `.exe`, `.msi`, `.zip`, and `.checksums.txt` assets from stable `vX.Y.Z` tags; current install-folder releases are self-contained, the Burn setup EXE is the intended winget installer, and `build/Export-WingetManifest.ps1` should prefer the published `-setup.exe` asset while falling back to the MSI for older releases; source winget hash and ARP metadata from the same published release assets, not local `release-*-buildoutput` probes; historical framework-dependent MSI manifests should use `Microsoft.DotNet.Runtime.10` rather than `Microsoft.DotNet.DesktopRuntime.10`; generate setup-bundle and WiX/MSI winget manifests with `ElevationRequirement: elevatesSelf` so the installer prompts for UAC itself instead of tripping the current non-elevated winget MSI launch bug; local manifest validation on `winget v1.29.140-preview` is currently confounded by the no-extension/MOTW client bug, so prefer `v1.29.160-preview` or newer for local manifest install tests and treat direct setup EXE runs as the package-side validation signal on older clients; unsigned EXE/MSI artifacts are accepted by winget but can still be blocked by SmartScreen in real installs, so signing the published setup EXE remains the safer release path; local silent upgrade failures like `1603` or `Error 1730` are usually admin-related validation limits, not proof of a bad rebuild; keep `ICE03` suppressed specifically in `build/CoolWSL.Installer.wixproj`; keep `RemoveFolder Directory="CoolWSLProgramMenuFolder" On="uninstall"` in the shortcut-owning component to satisfy `ICE64`.
