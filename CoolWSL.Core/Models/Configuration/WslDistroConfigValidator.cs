...
public static class WslDistroConfigValidator
{
        ...
        switch (schemaKey.ValueType)
        {
            case WslConfigValueType.Boolean:
                if (!string.Equals(entry.Value, "true", StringComparison.OrdinalIgnoreCase) && 
                    !string.Equals(entry.Value, "false", StringComparison.OrdinalIgnoreCase))
                {
            ...
            case WslConfigValueType.Integer:
                if (!uint.TryParse(entry.Value, out _))
                {
            ...
            case WslConfigValueType.OctalMask:
                if (!OctalRegex.IsMatch(entry.Value))
                {
            ...
            case WslConfigValueType.Hostname:
                if (!HostnameRegex.IsMatch(entry.Value))
                {
            ...
            case WslConfigValueType.LinuxUsername:
                if (!UsernameRegex.IsMatch(entry.Value) || entry.Value.Length > 32)
                {
            ...
            case WslConfigValueType.Enum:
                if (schemaKey.AllowedValues == null || !schemaKey.AllowedValues.Contains(entry.Value, StringComparer.OrdinalIgnoreCase))
                {
    ...
    {
        var tokens = entry.Value.Split(',', StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries);
